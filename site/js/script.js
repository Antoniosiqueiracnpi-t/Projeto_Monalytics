...
let mercSecData = null;
let currentIndexador = 'ipca';
let currentView = 'table';
let tableExpanded = false;

/**
 * Carrega dados do mercado secundário
 */
async function loadMercadoSecundario() {
    try {
        console.log('📊 Carregando dados do mercado secundário...');
        
        const response = await fetch(
            'https://raw.githubusercontent.com/Antoniosiqueiracnpi-t/Projeto_Monalytics/main/site/data/portfolio_renda_fixa.json?t=' + Date.now(),
        ...
        mercSecData = await response.json();
        console.log('✅ Dados do mercado secundário carregados:', mercSecData);
        
        renderMercadoSecundario();
        
    } catch (error) {
        console.error('❌ Erro ao carregar mercado secundário:', error);
        document.getElementById('mercSecLoading').innerHTML = `
            <i class="fas fa-exclamation-triangle"></i>
...
/**
 * Renderiza todos os componentes do mercado secundário
 */
function renderMercadoSecundario() {
    if (!mercSecData) return;
    
    // Esconde loading
    document.getElementById('mercSecLoading').style.display = 'none';
    document.getElementById('mercSecContent').style.display = 'flex';
    
    // Timestamp
    const timestamp = new Date(mercSecData.gerado_em);
    document.getElementById('mercSecTimestamp').textContent = 
        `Atualizado em ${timestamp.toLocaleDateString('pt-BR')} às ${timestamp.toLocaleTimeString('pt-BR', { hour: '2-digit', minute: '2-digit' })}`;
...
/**
 * Renderiza tabela resumo por indexador
 */
function renderResumoTable() {
    const tbody = document.getElementById('mercSecResumoTable');
    if (!tbody || !mercSecData.resumo) return;
    
    tbody.innerHTML = mercSecData.resumo.map(row => {
        const isTotal = row.Indexador === 'TOTAL';
        const distribuicao = row['Distribuição (%)'] !== null 
            ? row['Distribuição (%)'].toFixed(2) + '%' 
            : '-';
        const puPar = row['PU/Par Médio (%)'] !== null 
            ? row['PU/Par Médio (%)'].toFixed(2) + '%' 
            : '-';
        const duration = row['Duration Média (dias)'] !== null 
            ? Math.round(row['Duration Média (dias)']) + ' dias' 
            : '-';
        const taxaMedia = row['Taxa Indicativa Média (%)'] !== null 
            ? row['Taxa Indicativa Média (%)'].toFixed(2) + '%' 
            : '-';
        const maiorTaxa = row['Maior Taxa (%)'] !== null 
            ? row['Maior Taxa (%)'].toFixed(2) + '%' 
            : '-';
        
        return `
            <tr>
                <td><strong>${row.Indexador}</strong></td>
                <td>${distribuicao}</td>
...
/**
 * Renderiza tabela Top 10
 */
function renderTop10Table() {
    const tbody = document.getElementById('top10TableBody');
    if (!tbody) return;
    
    const key = currentIndexador === 'ipca' ? 'ipca_spread' : 'di_spread';
    const top10 = mercSecData.top10[key] || [];
    
    tbody.innerHTML = top10.map((item, index) => {
        const isCollapsed = index >= 5;
        return `
            <tr class="${isCollapsed ? 'collapsed-row' : ''}">
                <td><strong>#${index + 1}</strong></td>
                <td><strong>${item.id}</strong></td>
                <td style="color: #fbbf24;">${item.spread.toFixed(2)}%</td>
                <td>${Math.round(item.duration)} dias</td>
            </tr>
        `;
    }).join('');
    
    // Reset botão expandir
    const expandBtn = document.getElementById('expandTableBtn');
    if (expandBtn) {
        expandBtn.classList.remove('expanded');
        expandBtn.innerHTML = '<i class="fas fa-chevron-down"></i> Ver todos os 10 títulos';
    }
    tableExpanded = false;
}
...
function renderTop10Chart() {
    const canvas = document.getElementById('top10ScatterChart');
    if (!canvas) return;
    ...
    // Destroi gráfico anterior se existir
    if (window.mercSecChart) {
        window.mercSecChart.destroy();
    }
    
    const chartData = {
        datasets: [{
            label: currentIndexador === 'ipca' ? 'IPCA+ Spread' : 'DI+ Spread',
            data: top10.map(item => ({
                x: item.duration,
                y: item.spread,
                label: item.id
            })),
            backgroundColor: currentIndexador === 'ipca' ? 'rgba(96, 165, 250, 0.6)' : 'rgba(251, 191, 36, 0.6)',
            borderColor: currentIndexador === 'ipca' ? '#60a5fa' : '#fbbf24',
            borderWidth: 2,
            pointRadius: 8,
            pointHoverRadius: 12
        }]
    ...
    window.mercSecChart = new Chart(ctx, {
        type: 'scatter',
        data: chartData,
        options: {
            responsive: true,
            ...
            plugins: {
                ...
                tooltip: {
                    ...
                    callbacks: {
                        ...
                        label: (item) => [
                            `Spread: ${item.parsed.y.toFixed(2)}%`,
                            `Duration: ${Math.round(item.parsed.x)} dias`
                        ]
                    }
                }
            },
...
/**
 * Inicializa controles do mercado secundário
 */
function initMercSecControls() {
    // Tabs IPCA/DI
    document.querySelectorAll('.top10-tab').forEach(tab => {
        tab.addEventListener('click', () => {
            ...
            currentIndexador = tab.dataset.indexador;
    ...
    document.querySelectorAll('.view-btn').forEach(btn => {
        btn.addEventListener('click', () => {
            ...
            const tableView = document.getElementById('top10TableView');
            const chartView = document.getElementById('top10ChartView');
    ...
    if (expandBtn) {
        expandBtn.addEventListener('click', () => {
            ...
            expandBtn.innerHTML = tableExpanded 
                ? '<i class="fas fa-chevron-up"></i> Recolher tabela'
                : '<i class="fas fa-chevron-down"></i> Ver todos os 10 títulos';
        });
    }
}
...
// Adiciona ao carregamento inicial da página
document.addEventListener('DOMContentLoaded', () => {
    // Aguarda a seção de renda fixa estar visível
    const observer = new IntersectionObserver((entries) => {
        entries.forEach(entry => {
            if (entry.isIntersecting && !mercSecData) {
                loadMercadoSecundario();
            }
        });
    }, { threshold: 0.1 });
    
    const mercSecContainer = document.querySelector('.mercado-secundario-container');
    if (mercSecContainer) {
        observer.observe(mercSecContainer);
    }
});
