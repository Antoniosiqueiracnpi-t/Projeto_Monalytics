...
def _stmt_kind(df: pd.DataFrame) -> Optional[str]:
...
def _discover_bpa_bpp_files(ticker_dir: Path) -> Dict[str, Dict[str, Optional[Path]]]:
    out = {
    ...
    tri_cands = {"bpa": [], "bpp": []}  # list[(score, path)]
    anu_cands = {"bpa": [], "bpp": []}
    ...
    for f in files:
        ...
        kind = _stmt_kind(dfh)
        if kind not in ("bpa", "bpp"):
            continue

        tri_cands[kind].append((_score_periodicity(dfh, f, "trimestral"), f))
        anu_cands[kind].append((_score_periodicity(dfh, f, "anual"), f))

    for kind in ("bpa", "bpp"):
        tri_cands[kind].sort(key=lambda x: x[0], reverse=True)
        anu_cands[kind].sort(key=lambda x: x[0], reverse=True)

        tri_best = tri_cands[kind][0][1] if tri_cands[kind] and tri_cands[kind][0][0] > 0 else None
        anu_best = anu_cands[kind][0][1] if anu_cands[kind] and anu_cands[kind][0][0] > 0 else None

        if tri_best and anu_best and tri_best.resolve() == anu_best.resolve():
            if len(anu_cands[kind]) > 1 and anu_cands[kind][1][0] > 0:
                anu_best = anu_cands[kind][1][1]

        out[kind]["trimestral"] = tri_best
        out[kind]["anual"] = anu_best

    return out


def _pick_original_named(tdir: Path) -> Dict[str, Dict[str, Optional[Path]]]:
...
def main():
    ...
    for tdir in tickers:
        ...
        try:
            original = _pick_original_named(tdir)
            need_fallback = any(
                original[k][p] is None for k in ("bpa", "bpp") for p in ("trimestral", "anual")
            )
            fallback = _discover_bpa_bpp_files(tdir) if need_fallback else {
                "bpa": {"trimestral": None, "anual": None},
                "bpp": {"trimestral": None, "anual": None},
            }

            # BPA
            bpa_tri = original["bpa"]["trimestral"] or fallback["bpa"]["trimestral"]
            bpa_anu = original["bpa"]["anual"] or fallback["bpa"]["anual"]
            if bpa_tri and bpa_anu:
                ...
                out_path = tdir / "bpa_padronizada.csv"
            ...
            # BPP
            bpp_tri = original["bpp"]["trimestral"] or fallback["bpp"]["trimestral"]
            bpp_anu = original["bpp"]["anual"] or fallback["bpp"]["anual"]
            if bpp_tri and bpp_anu:
                ...
                out_path = tdir / "bpp_padronizada.csv"
                out_bpp.to_csv(out_path, index=False)
                gerados.append(str(out_path))
            else:
                print(f"[{ticker}] BPP: arquivos não encontrados (trimestral/anual). Pulando BPP.")

            print(f"[OK] {ticker} (BPA/BPP) finalizado.")
        except Exception as e:
            erros += 1
            print(f"[ERRO] Falha ao processar BPA/BPP de {ticker}: {e}")
            traceback.print_exc()
