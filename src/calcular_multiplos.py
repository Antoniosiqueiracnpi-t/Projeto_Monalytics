...
"""
Calculadora de Múltiplos Financeiros para Empresas Brasileiras
==============================================================
VERSÃO 2.0 CORRIGIDA - Janeiro 2025

Calcula múltiplos organizados por tipo de empresa:

EMPRESAS NÃO-FINANCEIRAS (24 múltiplos):
- Valuation (8): Valor de Mercado, P/L, P/VPA, EV/EBITDA, EV/EBIT, EV/Receita, DY, Payout
...
BANCOS (9 múltiplos):
- Valuation (5): Valor de Mercado, P/L, P/VPA, DY, Payout
- Rentabilidade (3): ROE, ROA, Margem Líquida
- Estrutura (1): PL/Ativos

HOLDINGS DE SEGUROS (11 múltiplos):
- Valuation (6): Valor de Mercado, P/L, P/VPA, DY, Payout, EV/Receita
- Rentabilidade (4): ROE, ROA, Margem Líquida, Margem Operacional
- Eficiência (1): Índice de Eficiência

SEGURADORAS OPERACIONAIS (11 múltiplos):
- Valuation (5): Valor de Mercado, P/L, P/VPA, DY, Payout
...
CORREÇÕES v2.0:
1. DY e PAYOUT usam DPA diretamente (sem conversão dupla)
2. ROIC usa Capital Investido = PL + Dívida Bruta (não líquida)
3. Giro do Ativo usa Ativo Total MÉDIO
4. PME usa Estoque MÉDIO
5. ICJ com busca flexível de despesas financeiras
6. Bancos usam conta 3.99 (LPA) quando disponível
7. BUG corrigido: pl_code em holdings de seguros
8. Dividendos LTM simplificado e robusto

FÓRMULAS ALINHADAS COM:
- StatusInvest
- Investidor10
- InvestSite
...
CONTAS_DRE = {
    ...
    "despesas_financeiras": "3.06.02",  # Despesas financeiras específicas
    "lucro_liquido": "3.11",
    "ir_csll": "3.08",
    "lpa": "3.99",  # Lucro por Ação (calculado pela CVM)
}
...
# Taxa de IR para NOPAT (alíquota efetiva média brasileira)
...
def _safe_divide(numerador: float, denominador: float, default: float = np.nan, eps: float = 1e-9) -> float:
    """Divisão segura que retorna default se denominador for zero ou valores inválidos."""
...
def _series_to_numeric_smart(ser: pd.Series) -> pd.Series:
    """Converte série para numérico com tolerância a formatos."""
...
def _encontrar_periodo_imputacao(df: pd.DataFrame, periodo_req: str) -> Optional[str]:
    """
    Encontra período substituto para preencher lacunas.
    ...
    Lógica:
        ...
        3. Se não houver anterior, tenta posterior mais próximo (Backfill)
    """
...
def _detectar_coluna_especie(df: pd.DataFrame) -> Optional[str]:
    """Detecta coluna de espécie de ação no DataFrame."""
    if df is None:
        return None
    candidatos = {"espécie_acao", "especie_acao", "espécie_ação", "especie", "espécie", "Espécie_Acao"}
...
def _obter_acoes_especie(dados: DadosEmpresa, especie: str, periodo: str) -> float:
    ...
    Robustez: tolera formato pt-BR/EN e usa imputação de período.
    """
...
def _calcular_fator_unit_pacote(dados: DadosEmpresa, periodo: str) -> int:
    ...
    Regra: fator = (ações ON+PN) / (quantidade de UNIT)
...
def _ajustar_acoes_para_ticker_preco(dados: DadosEmpresa, periodo: str, ticker_preco: Optional[str]) -> Tuple[float, str, int]:
    ...
    Se ticker_preco for UNIT (classe 11), converte ações totais (ON+PN) para "unidades de UNIT".
...
def _listar_tickers_saida_multiclasse(dados: DadosEmpresa, ticker_seed: str) -> List[str]:
    """
    Lista tickers (com classe) disponíveis no arquivo de preços para a mesma empresa.
...
def _obter_acoes(dados: DadosEmpresa, periodo: str) -> float:
    ...
    Regra geral: priorizar ON+PN (ignorar UNIT/11 quando possível).
    """
    if dados.acoes is None or dados.acoes.empty:
        return np.nan
...
def _calcular_market_cap_atual(dados: DadosEmpresa, ticker_preco: Optional[str] = None) -> float:
    ...
    # referência de período para ações
    periodo_ref = ""
...
def _calcular_ev(dados: DadosEmpresa, periodo: str, market_cap: Optional[float] = None) -> float:
    ...
    EV = VM + (Empréstimos CP + Empréstimos LP) - (Caixa + Aplicações)
    ...
    # Buscar empréstimos com códigos alternativos
    emp_cp = _buscar_conta_flexivel(dados.bpp, ["2.01.04", "2.01.04.01", "2.01.05"], periodo)
    emp_lp = _buscar_conta_flexivel(dados.bpp, ["2.02.01", "2.02.01.01", "2.02.02"], periodo)
...
def _calcular_ltm(
    ...
    """
    Calcula valor LTM (últimos 12 meses) para uma conta de DRE.
    
    Para empresas semestrais (AGRO3): usa 3 períodos
...
def _obter_valor_medio(
    ...
    Usado para ROE, ROA, Giro do Ativo, etc.
    
    FÓRMULA: (Valor Atual + Valor 4 trimestres atrás) / 2
    """
    if df is None:
        return np.nan
...
def _calcular_da_periodo(dados: DadosEmpresa, periodo: str) -> float:
    ...
    Prioridade:
    1) Código sintético 6.01.DA (se existir)
    2) Códigos específicos de D&A (6.01.01.02 e 6.01.01.01)
    3) Subcontas típicas de D&A
    4) NaN (não usa agregador 6.01.01 para evitar superestimar)
    """
    ...
    # 1) Preferência máxima: código sintético
    val = _val_exato("6.01.DA")
    if np.isfinite(val):
        return abs(val)

    # 2) Códigos específicos de D&A
...
def _calcular_ebitda_periodo(dados: DadosEmpresa, periodo: str) -> float:
    ...
    Se D&A não disponível, retorna EBIT como aproximação conservadora.
    """
...
# ======================================================================================
# CÁLCULO DE DIVIDENDOS - CORRIGIDO v2.0
# ======================================================================================

def _obter_dpa_periodo(dados: DadosEmpresa, periodo: str) -> float:
    """
    Obtém Dividendo por Ação (DPA) de um período específico.
    Busca direto no arquivo dividendos_trimestrais.csv.
    
    Returns:
        DPA em R$/ação ou np.nan se não disponível
    """
    ...
    if len(vals) > 0:
        ...
        return dpa if np.isfinite(dpa) and dpa >= 0 else np.nan
...
def _calcular_dpa_ltm(dados: DadosEmpresa, periodo_fim: str) -> float:
    ...
    CORREÇÃO v2.0: Soma DPA diretamente, sem conversão para total.
    ...
    # Montar lista de 4 períodos LTM
    periodos_12m = []
...
def _calcular_dividendos_ltm(dados: DadosEmpresa, periodo_fim: str) -> float:
    ...
    CORREÇÃO v2.0: Usa DPA LTM × Ações atuais, evitando conversões duplas.
    ...
    # DPA × Ações / 1000 = Dividendos em R$ mil
    return (dpa_ltm * acoes) / 1000.0


# ======================================================================================
# CÁLCULO DE LPA (Lucro por Ação)
# ======================================================================================
...
def _calcular_lpa_ltm(dados: DadosEmpresa, periodo_fim: str) -> float:
    """
    Calcula LPA LTM (anualizado) somando últimos 4 trimestres da conta 3.99.
    ...
    n_trimestres = 3 if padrao.tipo == 'SEMESTRAL' else 4
    start_idx = max(0, idx_fim - n_trimestres + 1)
    periodos_ltm = periodos[start_idx:idx_fim + 1]
...
def _obter_preco_ultimo_trimestre_ano(
    ...
    """
    Obtém o preço do último trimestre disponível do ANO do período de referência.
    """
    try:
...
# ======================================================================================
# FUNÇÕES AUXILIARES PARA HOLDINGS E SEGURADORAS
# ======================================================================================
...
def _obter_receita_holding_cxse3(dados: DadosEmpresa, periodo: str) -> float:
    """Obtém receita total da CXSE3: Serviços (3.04.04.02) + Equivalência (3.04.06)."""
...
def _detectar_codigo_pl_banco(df_bpp: pd.DataFrame) -> str:
    ...
    for idx, row in df_bpp.iterrows():
        ...
        if 'patrimônio líquido' in conta or 'patrimonio liquido' in conta:
            if cd.startswith('2.0') and len(cd) == 4:
                return cd
    return "2.07"


# ======================================================================================
# CALCULADORA DE MÚLTIPLOS - EMPRESAS NÃO-FINANCEIRAS (24 MÚLTIPLOS)
# VERSÃO 2.0 CORRIGIDA - Alinhada com StatusInvest/Investidor10
# ======================================================================================
...
def calcular_multiplos_periodo(dados: DadosEmpresa, periodo: str, usar_preco_atual: bool = True, ticker_preco: Optional[str] = None) -> Dict[str, Optional[float]]:
    """
    Calcula todos os 24 múltiplos para um período específico (empresas não-financeiras).
    
    CORREÇÕES v2.0:
    1. P/L e P/VPA: usa LPA e VPA por ação direto
    2. DY: usa DPA LTM / Preço × 100 (sem conversões)
    3. PAYOUT: usa DPA LTM / LPA LTM × 100
    4. ROIC: Capital Investido = PL + Dívida BRUTA
    5. Giro do Ativo: usa Ativo MÉDIO
    6. PME: usa Estoque MÉDIO
    7. ICJ: busca flexível de despesas financeiras
    ...
    Args:
        ...
        ticker_preco: Ticker específico para preços (multi-classe)
    ...
    # Ações para o ticker de preço
    acoes_ref, _periodo_acoes_ref, _fator_unit = _ajustar_acoes_para_ticker_preco(dados, periodo, ticker_preco)
    
    # Preço
    if usar_preco_atual:
        preco, periodo_preco = _obter_preco_atual(dados, ticker_preco=ticker_preco)
    else:
        preco, periodo_preco = _obter_preco_ultimo_trimestre_ano(dados, periodo, ticker_preco=ticker_preco)
    ...
    # P/L = Preço / LPA LTM
    # CORREÇÃO v2.0: Prioriza conta 3.99 (LPA calculado pela CVM)
    lpa_ltm = _calcular_lpa_ltm(dados, periodo)
    
    # Se não tem 3.99, calcula manualmente
    if not (np.isfinite(lpa_ltm) and lpa_ltm != 0):
    ...
    # P/VPA = Preço / VPA
    # VPA = (PL × 1000) / Ações
    if np.isfinite(pl) and np.isfinite(acoes_ref) and acoes_ref > 0:
        vpa = (pl * 1000.0) / acoes_ref
    else:
        vpa = np.nan
    ...
    # ==================== DIVIDENDOS (CORRIGIDO v2.0) ====================
    
    # DPA LTM (Dividendo por Ação dos últimos 12 meses)
    dpa_ltm = _calcular_dpa_ltm(dados, periodo)
    
    # DY = (DPA LTM / Preço) × 100
    # CORREÇÃO v2.0: Usa DPA diretamente, sem conversão para total
    resultado["DY"] = _normalizar_valor(_safe_divide(dpa_ltm, preco) * 100)
    
    # PAYOUT = (DPA LTM / LPA LTM) × 100
    # CORREÇÃO v2.0: Usa DPA e LPA diretamente
    resultado["PAYOUT"] = _normalizar_valor(_safe_divide(dpa_ltm, lpa_ltm) * 100)
    
    # ==================== RENTABILIDADE ====================
    
    # ROE = LL LTM / PL Médio × 100
    ...
    # CORREÇÃO v2.0: Capital Investido = PL + Dívida BRUTA (não líquida)
    ...
    # Capital Investido = PL + Dívida Bruta (fórmula StatusInvest)
    ...
    # ICJ = EBIT LTM / Despesas Financeiras LTM
    # CORREÇÃO v2.0: Busca flexível de despesas financeiras
    desp_fin_ltm = _calcular_ltm(dados, dados.dre, "3.06.02", periodo)
    if not np.isfinite(desp_fin_ltm):
        # Tentar resultado financeiro líquido (se negativo = despesa)
        res_fin_ltm = _calcular_ltm(dados, dados.dre, "3.06", periodo)
        if np.isfinite(res_fin_ltm) and res_fin_ltm < 0:
            desp_fin_ltm = abs(res_fin_ltm)
    ...
    # Composição Dívida = Emp CP / Dívida Bruta × 100
    ...
    # ==================== EFICIÊNCIA (CORRIGIDO v2.0) ====================
    
    # Giro do Ativo = Receita LTM / Ativo Total MÉDIO
    # CORREÇÃO v2.0: Usa Ativo MÉDIO (não pontual)
    resultado["GIRO_ATIVO"] = _normalizar_valor(_safe_divide(receita_ltm, at_medio))
    
    # PME = Estoque MÉDIO × 360 / |CPV LTM|
    # CORREÇÃO v2.0: Usa Estoque MÉDIO
    ...
    # Estoque médio
    estoques_medio = _obter_valor_medio(dados, dados.bpa, CONTAS_BPA["estoques"], periodo)
    ativos_bio_medio = _obter_valor_medio(dados, dados.bpa, CONTAS_BPA["ativos_biologicos"], periodo)
    estoques_medio_val = estoques_medio if np.isfinite(estoques_medio) else 0
    ativos_bio_medio_val = ativos_bio_medio if np.isfinite(ativos_bio_medio) else 0
    estoque_total_medio = estoques_medio_val + ativos_bio_medio_val
    
    cpv_ltm = _calcular_ltm(dados, dados.dre, CONTAS_DRE["cpv"], periodo)
    cpv_ltm_abs = abs(cpv_ltm) if np.isfinite(cpv_ltm) else np.nan
    
    resultado["PME"] = _normalizar_valor(_safe_divide(estoque_total_medio * 360, cpv_ltm_abs))
    
    # PMR = Contas a Receber × 360 / Receita LTM
    ...
    # NCG/Receita = (AC Operacional - PC Operacional) / Receita LTM × 100
...
MULTIPLOS_METADATA = {
    "VALOR_MERCADO": {"nome": "Valor de Mercado", "categoria": "Valuation", "formula": "Preço × Ações", "unidade": "R$ mil", "usa_preco": True},
    "P_L": {"nome": "P/L", "categoria": "Valuation", "formula": "Preço / LPA LTM", "unidade": "x", "usa_preco": True},
    "P_VPA": {"nome": "P/VPA", "categoria": "Valuation", "formula": "Preço / VPA", "unidade": "x", "usa_preco": True},
    "EV_EBITDA": {"nome": "EV/EBITDA", "categoria": "Valuation", "formula": "EV / EBITDA LTM", "unidade": "x", "usa_preco": True},
    "EV_EBIT": {"nome": "EV/EBIT", "categoria": "Valuation", "formula": "EV / EBIT LTM", "unidade": "x", "usa_preco": True},
    "EV_RECEITA": {"nome": "EV/Receita", "categoria": "Valuation", "formula": "EV / Receita LTM", "unidade": "x", "usa_preco": True},
    "DY": {"nome": "Dividend Yield", "categoria": "Valuation", "formula": "DPA LTM / Preço × 100", "unidade": "%", "usa_preco": True},
    "PAYOUT": {"nome": "Payout", "categoria": "Valuation", "formula": "DPA LTM / LPA LTM × 100", "unidade": "%", "usa_preco": False},
    "ROE": {"nome": "ROE", "categoria": "Rentabilidade", "formula": "LL LTM / PL Médio × 100", "unidade": "%", "usa_preco": False},
    "ROA": {"nome": "ROA", "categoria": "Rentabilidade", "formula": "LL LTM / Ativo Médio × 100", "unidade": "%", "usa_preco": False},
    "ROIC": {"nome": "ROIC", "categoria": "Rentabilidade", "formula": "NOPAT / (PL + Dív. Bruta) × 100", "unidade": "%", "usa_preco": False},
    "MARGEM_EBITDA": {"nome": "Margem EBITDA", "categoria": "Rentabilidade", "formula": "EBITDA / Receita × 100", "unidade": "%", "usa_preco": False},
    "MARGEM_LIQUIDA": {"nome": "Margem Líquida", "categoria": "Rentabilidade", "formula": "LL / Receita × 100", "unidade": "%", "usa_preco": False},
    "DIV_LIQ_EBITDA": {"nome": "Dív.Líq/EBITDA", "categoria": "Endividamento", "formula": "Dívida Líquida / EBITDA", "unidade": "x", "usa_preco": False},
    "DIV_LIQ_PL": {"nome": "Dív.Líq/PL", "categoria": "Endividamento", "formula": "Dívida Líquida / PL", "unidade": "x", "usa_preco": False},
    "ICJ": {"nome": "ICJ", "categoria": "Endividamento", "formula": "EBIT / Desp. Financeiras", "unidade": "x", "usa_preco": False},
    "COMPOSICAO_DIVIDA": {"nome": "Composição Dívida", "categoria": "Endividamento", "formula": "Dív. CP / Dív. Total × 100", "unidade": "%", "usa_preco": False},
    "LIQ_CORRENTE": {"nome": "Liquidez Corrente", "categoria": "Liquidez", "formula": "AC / PC", "unidade": "x", "usa_preco": False},
    ...
    "GIRO_ATIVO": {"nome": "Giro do Ativo", "categoria": "Eficiência", "formula": "Receita LTM / Ativo Médio", "unidade": "x", "usa_preco": False},
    "CICLO_CAIXA": {"nome": "Ciclo de Caixa", "categoria": "Eficiência", "formula": "PMR + PME - PMP", "unidade": "dias", "usa_preco": False},
    "PME": {"nome": "PME", "categoria": "Eficiência", "formula": "Estoque Médio × 360 / CPV", "unidade": "dias", "usa_preco": False},
    "NCG_RECEITA": {"nome": "NCG/Receita", "categoria": "Eficiência", "formula": "NCG / Receita × 100", "unidade": "%", "usa_preco": False},
}


# ======================================================================================
# METADADOS E CALCULADORA - BANCOS (9 MÚLTIPLOS)
# ======================================================================================
...
MULTIPLOS_BANCOS_METADATA = {
    ...
    "ROE": {"nome": "ROE", "categoria": "Rentabilidade", "formula": "LL LTM / PL Médio × 100", "unidade": "%", "usa_preco": False},
    "ROA": {"nome": "ROA", "categoria": "Rentabilidade", "formula": "LL LTM / Ativo Médio × 100", "unidade": "%", "usa_preco": False},
    "MARGEM_LIQUIDA": {"nome": "Margem Líquida", "categoria": "Rentabilidade", "formula": "LL LTM / Rec. Interm. LTM × 100", "unidade": "%", "usa_preco": False},
    "PL_ATIVOS": {"nome": "PL/Ativos", "categoria": "Estrutura", "formula": "PL / Ativo Total × 100", "unidade": "%", "usa_preco": False},
}
...
def calcular_multiplos_banco(dados: DadosEmpresa, periodo: str, usar_preco_atual: bool = True, ticker_preco: Optional[str] = None) -> Dict[str, Optional[float]]:
    """
    Calcula 9 múltiplos essenciais para bancos.
    
    CORREÇÃO v2.0: Usa conta 3.99 (LPA) quando disponível.
    """
    resultado: Dict[str, Optional[float]] = {}
    ...
    # Ações e preço
    acoes_ref, _, _ = _ajustar_acoes_para_ticker_preco(dados, periodo, ticker_preco)
    
    if usar_preco_atual:
        preco, _ = _obter_preco_atual(dados, ticker_preco=ticker_preco)
    else:
        preco, _ = _obter_preco_ultimo_trimestre_ano(dados, periodo, ticker_preco=ticker_preco)
    
    # ==================== VALUATION ====================
    
    # P/L - CORREÇÃO v2.0: Tenta usar conta 3.99 primeiro
    lpa_ltm = _calcular_lpa_ltm(dados, periodo)
    if not (np.isfinite(lpa_ltm) and lpa_ltm != 0):
    ...
    # DY e PAYOUT - CORREÇÃO v2.0: Usa DPA diretamente
    dpa_ltm = _calcular_dpa_ltm(dados, periodo)
    resultado["DY"] = _normalizar_valor(_safe_divide(dpa_ltm, preco) * 100)
    resultado["PAYOUT"] = _normalizar_valor(_safe_divide(dpa_ltm, lpa_ltm) * 100)
...
# ======================================================================================
# METADADOS E CALCULADORA - HOLDINGS DE SEGUROS (11 MÚLTIPLOS)
# ======================================================================================
...
MULTIPLOS_HOLDINGS_SEGUROS_METADATA = {
    ...
    "ROE": {"nome": "ROE", "categoria": "Rentabilidade", "formula": "LL LTM / PL Médio × 100", "unidade": "%", "usa_preco": False},
    "ROA": {"nome": "ROA", "categoria": "Rentabilidade", "formula": "LL LTM / Ativo Médio × 100", "unidade": "%", "usa_preco": False},
    "MARGEM_LIQUIDA": {"nome": "Margem Líquida", "categoria": "Rentabilidade", "formula": "LL LTM / Receita LTM × 100", "unidade": "%", "usa_preco": False},
    "MARGEM_OPERACIONAL": {"nome": "Margem Operacional", "categoria": "Rentabilidade", "formula": "EBIT / Receita LTM × 100", "unidade": "%", "usa_preco": False},
    "INDICE_EFICIENCIA": {"nome": "Índice Eficiência", "categoria": "Eficiência", "formula": "Despesas / Receita × 100", "unidade": "%", "usa_preco": False},
}
...
def calcular_multiplos_holding_seguros(dados: DadosEmpresa, periodo: str, usar_preco_atual: bool = True, ticker_preco: Optional[str] = None) -> Dict[str, Optional[float]]:
    ...
    CORREÇÃO v2.0: Definição correta de pl_code.
    """
    resultado: Dict[str, Optional[float]] = {}
    ...
    if ticker_upper == "BBSE3":
        ll_ltm = _calcular_ltm(dados, dados.dre, "3.13", periodo)
    else:  # CXSE3
    ...
    # Receita LTM específica por ticker
    if ticker_upper == "BBSE3":
    ...
    # EBIT LTM
    if ticker_upper == "BBSE3":
        ebit_ltm = _calcular_ltm(dados, dados.dre, "3.07", periodo)
    else:
    ...
    # P/VPA - CORREÇÃO v2.0: Usa conta padrão 2.03, não pl_code indefinido
    pl = _obter_valor_pontual(dados.bpp, CONTAS_BPP["patrimonio_liquido"], periodo)
    pl_medio = _obter_valor_medio(dados, dados.bpp, CONTAS_BPP["patrimonio_liquido"], periodo)
...
# ======================================================================================
# METADADOS E CALCULADORA - SEGURADORAS OPERACIONAIS (11 MÚLTIPLOS)
# ======================================================================================
...
MULTIPLOS_SEGURADORAS_METADATA = {
    ...
    "ROA": {"nome": "ROA", "categoria": "Rentabilidade", "formula": "LL LTM / Ativo Médio × 100", "unidade": "%", "usa_preco": False},
    "PL_ATIVOS": {"nome": "PL/Ativos", "categoria": "Estrutura", "formula": "PL / Ativo Total × 100", "unidade": "%", "usa_preco": False},
    "COMBINED_RATIO": {"nome": "Combined Ratio", "categoria": "Operacional", "formula": "Despesas / Prêmios × 100", "unidade": "%", "usa_preco": False},
    "SINISTRALIDADE": {"nome": "Sinistralidade", "categoria": "Operacional", "formula": "Sinistros / Prêmios × 100", "unidade": "%", "usa_preco": False},
    "MARGEM_SUBSCRICAO": {"nome": "Margem Subscrição", "categoria": "Operacional", "formula": "Res. Bruto / Prêmios × 100", "unidade": "%", "usa_preco": False},
}
...
def calcular_multiplos_seguradora(dados: DadosEmpresa, periodo: str, usar_preco_atual: bool = True, ticker_preco: Optional[str] = None) -> Dict[str, Optional[float]]:
    """
    Calcula 11 múltiplos para seguradoras operacionais (IRBR3, PSSA3).
    Suporta estrutura IFRS 17 (2023+).
    """
    resultado: Dict[str, Optional[float]] = {}
    ticker_upper = dados.ticker.upper().strip()
    ...
    if ticker_upper == "IRBR3":
        ...
        total_despesas = sinistros_ltm + custos_aq_ltm + desp_admin_ltm
    ...
    resultado["COMBINED_RATIO"] = _normalizar_valor(
        _safe_divide(total_despesas, premios_ltm) * 100 if np.isfinite(premios_ltm) and premios_ltm > 0 else np.nan
    )
...
def gerar_historico_anualizado(dados: DadosEmpresa, ticker_preco: Optional[str] = None, ticker_saida: Optional[str] = None) -> Dict[str, Any]:
    ...
    for ano in sorted(periodos_por_ano.keys()):
        ...
        # Histórico usa preço do período (não atual)
...
# ======================================================================================
# PROCESSADOR PRINCIPAL E FUNÇÕES DE SALVAMENTO
# ======================================================================================

def _salvar_csv_historico(resultado: Dict, path: Path):
    """Salva histórico em formato CSV."""
...
def _salvar_js_historico(resultado: Dict[str, Any], output_path: Path, ticker: Optional[str] = None) -> None:
    """Salva resultado como JavaScript."""
    try:
...
def processar_ticker(ticker: str, salvar: bool = True) -> Tuple[bool, str, Optional[Dict]]:
    ...
    Gera arquivos por classe quando disponíveis múltiplas classes.
    """
...
def main():
    parser = argparse.ArgumentParser(
        description="Calculadora de Múltiplos Financeiros v2.0"
    )
    ...
    print(f"\n{'='*70}")
    print(f">>> CALCULADORA DE MÚLTIPLOS FINANCEIROS v2.0 <<<")
    print(f"{'='*70}")
    print(f"Modo: {args.modo} | Selecionadas: {len(df_sel)}")
    print(f"Empresas: 24 múltiplos | Bancos: 9 | Holdings Seguros: 11 | Seguradoras: 11")
    print(f"Saída: balancos/<TICKER>/multiplos_<TICKER>.js + multiplos_<TICKER>.csv")
    print(f"{'='*70}\n")
    ...
    print(f"\n{'='*70}")
    print(f"RESUMO: OK={ok_count} | SKIP={skip_count} | ERRO={err_count}")
    print(f"{'='*70}\n")
