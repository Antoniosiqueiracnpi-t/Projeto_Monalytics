...
def _fiscal_year(dt: pd.Timestamp, fiscal_end_month: int) -> int:
    # Ano fiscal é o ano do fechamento do exercício (mês fiscal_end_month)
    if pd.isna(dt):
        return -1
...
class PadronizadorDRE:
    ...
    def _to_isolated_quarters(self, qtot: pd.DataFrame, cumulative_years: Dict[int, bool]) -> pd.DataFrame:
        """
        Se cumulativo: isola por diferença Tn - T(n-1) (somente se sequência sem gaps).
        EPS não é convertido (fica como veio).
        """
        out_rows = []
    ...
    def _add_t4_from_annual_when_missing(self, qiso: pd.DataFrame, anual: pd.DataFrame) -> pd.DataFrame:
        """
        Se tiver T1,T2,T3 e NÃO tiver T4, cria T4 = anual - (T1+T2+T3).
        Se já tiver T4 no trimestral => mantém como está.
        """
        anual_map = anual.set_index(["fiscal_year", "code"])["anual_val"].to_dict()
        out = qiso.copy()
    ...
    def _build_horizontal(self, qiso: pd.DataFrame) -> pd.DataFrame:
        # colunas (períodos) do mais antigo para o mais recente
        periods = (
    ...
    def _checkup_vs_anual(self, qiso: pd.DataFrame, anual: pd.DataFrame) -> Tuple[int, int, int]:
        """
        Check-up interno (não salva arquivo):
          - OK: soma T1..T4 bate com anual (tolerância)
          - INCOMPLETO_TRIMESTRAL: falta algum trimestre para comparar
          - SEM_ANUAL: não tem anual para comparar
        Retorna contadores: (diverge, incompleto, sem_anual)
        """
        anual_map = anual.set_index(["fiscal_year", "code"])["anual_val"].to_dict()
    ...
    def padronizar_e_salvar_ticker(self, ticker: str) -> Tuple[bool, str]:
        """
        Salva SOMENTE:
          balancos/<ticker>/dre_padronizado.csv
        ...
        # Check-up interno (não salva nada)
        ...
        ok = (diverge == 0)  # ok “estrito” apenas para log
        return ok, msg
...
def main():
    ...
    # idêntico ao captura_simples.py
    ...
    for _, row in df_sel.iterrows():
        ...
        # exige que já exista captura no diretório balancos
